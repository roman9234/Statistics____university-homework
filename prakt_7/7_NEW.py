...
print(f"Выборочная дисперсия от фактора случайности S_0_squared: {S_0_squared}")
print("Задание 2-3")

s_nA_squared = (m / (k - 1)) * part_2
print(f"Выборочная дисперсия включающая влияние фактора A и дисперсию от фактора случайности s_nA_squared: {round(s_nA_squared,2)}")
s_nB_squared = (k / (m - 1)) * part_1_2
print(f"Выборочная дисперсия включающая влияние фактора B и дисперсию от фактора случайности s_nB_squared: {round(s_nB_squared,2)}")
...
# F - критерий.
# Если рассчитанное отношение дисперсий больше критического значения F, фактор А считается значимым.
...
F = stats.f.ppf(0.95, (k - 1), (k - 1) * (m - 1))

# Уровень значимости 0.05 - веротяность что мы ошибочно отвергнем нулевую гипотезу
# 0.95 - доверительная вероятность. Вероятность того, что мы не отвергнем нулевую гипотезу, если она истинна.

s = 0
SA = 0
SB = 0

print(f"Соотношение = {s_nA_squared/S_0_squared}")
print(f"F для A = {F}")
...
print(f"Соотношение = {s_nB_squared/S_0_squared}")
print(f"F для B = {F}")
...
if s != 0:
    print(f"Оценка генеральной дисперсии: {round(s,4)}")
